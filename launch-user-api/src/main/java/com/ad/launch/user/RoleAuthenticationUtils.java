...
public final class RoleAuthenticationUtils {
    /**
     * 实际使用的 {@link AdSimpleGrantedAuthority} 是不可变的
     */
    public static final Map<AuthenticationEnum, Set<AdGrantedAuthority>> CACHE;
...
//                    查找权重比当前 a 权重低的
                    Set<AdGrantedAuthority> authorities=Stream.of(AuthenticationEnum.values())
...
//                    补上当前 a
     ...
     */
    public static Collection<? extends AdGrantedAuthority> forGrantedAuthorities(AuthenticationEnum authenticationEnum) {
        Collection<? extends AdGrantedAuthority> grantedAuthorities=CACHE.get(authenticationEnum);
     ...
     */
    public static Collection<? extends AdGrantedAuthority> forGrantedAuthorities(String authentication) {
     ...
     */
    public static String grantedAuthorities2SingleString(Collection<? extends AdGrantedAuthority> grantedAuthorities) {
...
//                按照 {getOrdinal }降序
    ...
    public static <T> boolean isAuthentication(Collection<T> authorities, Function<String, T> createApply) {

        return Stream.of(AuthenticationEnum.values())
//                按照 {getOrdinal }降序
                .sorted(Comparator.comparingInt(AuthenticationEnum::getOrdinal).reversed())
                .filter(a->a.getOrdinal() >= AuthenticationEnum.ADMIN.getOrdinal())
                .anyMatch(a->authorities.contains(createApply.apply(authenticationEnum2GrantedAuthorityValue(a))));
    ...
    public static String[] authentication2ValueStringList(AuthenticationEnum authenticationEnum) {
        return CACHE.get(authenticationEnum).stream()
                .map(AdGrantedAuthority::getAuthority)
    ...
    private static AdSimpleGrantedAuthority authenticationEnum2GrantedAuthority(AuthenticationEnum authenticationEnum) {
        return new AdSimpleGrantedAuthority(authenticationEnum2GrantedAuthorityValue(authenticationEnum));
    }
